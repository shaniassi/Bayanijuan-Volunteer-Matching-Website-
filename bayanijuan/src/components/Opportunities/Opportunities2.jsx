            ...
            <div className='flex flex-wrap justify-center gap-8'>
                {categories.map((category, index) => (
                    <div key={index} className='bg-sun shadow-lg rounded-lg p-6 w-full md:w-[300px] flex flex-col items-center text-center h-[330px] mt-5'>
                        ...
                        <Link to={category.link} className='mt-6 bg-rose text-cream font-header text-2xl rounded-full px-10 py-1 hover:bg-opacity-80 transition duration-300 self-end'>Explore</Link>
                    </div>
                ))}
            </div>
        </div>
